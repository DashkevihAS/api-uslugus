...
function makeItemsFromData(data, id) {
  ...
  if (
    !/^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[\W]).{6,}/.test(item.password)
  ) {
    errors.push({
      field: "password",
      message:
        "Минимальная длина паролья 6 символов, обязательно содержит специмвол число и латинские буквы в нижнем и вернем регистре",
    });
  }
  ...
  if (item.about && item.about.length < 40) {
    errors.push({
      field: "about",
      message: "Минимальная длина описания 40 символов",
    });
  }
...
function findItems(reqData) {
  ...
  if (item.password !== reqData.password)
    throw new ApiError(404, { message: "Wrong password" });
  delete item.password;
  return item;
}
...
module.exports = server = createServer(async (req, res) => {
  ...
  .on("listening", () => {
    if (process.env.NODE_ENV !== "test") {
      ...
      console.log(
        `GET ${URI_PREFIX}?{search=""} - поиск услуги по имени и описанию`
      );
      console.log(
        `POST ${URI_PREFIX}/signup - Зарегистрировать специалиста, в теле запроса нужно передать объект`
      );
      console.log(
        `POST ${URI_PREFIX}/signin - Псевдоавторизация, отправьте email и пароль`
      );
